...
export function cleanTopicTitle(input) {
    ...
    if (parts.length >= 2) {
        ...
        const looksLikeIndexTail = /(diagnosis|classification|treatment|management|approach|evaluation|epidemiology|prevention|therapy|surgery|complications|pregnancy|travelers|stockpile|synovectomy)/i.test(tail);
        if (looksLikeIndexTail)
            s = parts[0];
    }
 ...
 *   apiGet<T>(path, { signal }?)
 */
async function apiGetSafe(path, signal) {
    const fn = apiGet;
    try {
...
export async function suggestByCollection(collection, q, limit = 20, signal) {
    const qs = new URLSearchParams();
    qs.set("q", q);
    qs.set("limit", String(limit));
    // Path WITHOUT /api/ prefix - http.ts adds /api base
    const data = await apiGetSafe(`/suggest/${collection}?${qs.toString()}`, signal);
    let list = [];
    if (Array.isArray(data))
        list = data.map(String);
    else if (data && typeof data === "object" && Array.isArray(data.suggestions)) {
        list = data.suggestions.map(String);
    }
...
export async function getTopicByCollection(collection, q, signal) {
    const qs = new URLSearchParams();
    qs.set("q", q);
    // Path WITHOUT /api/ prefix - http.ts adds /api base
    return await apiGetSafe(`/topic/${collection}?${qs.toString()}`, signal);
}
