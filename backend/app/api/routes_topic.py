...
def _add_timing_headers(response: Response, request: Request, result: dict) -> None:
    """Add X-Cache and timing headers to response."""
    ...
    total_ms = getattr(request.state, "timings", None)
    if total_ms is not None:
        total_value = total_ms.duration_ms("total")
...
async def topic_get(topic_id: str, request: Request, response: Response, debug: bool = Query(False)):
...
async def topic_medicine(request: Request, response: Response, q: str = Query(""), debug: bool = Query(False)):
    """Get medicine topic."""
...
async def topic_obgyn(request: Request, response: Response, q: str = Query(""), debug: bool = Query(False)):
    """Get OBGYN topic."""
    topic_id = q or "obgyn"
    result = await get_topic(topic_id, debug=debug)
    _add_timing_headers(response, request, result)
...
async def topic_pediatrics(request: Request, response: Response, q: str = Query(""), debug: bool = Query(False)):
...
async def topic_surgery(request: Request, response: Response, q: str = Query(""), debug: bool = Query(False)):
    """Get surgery topic."""
