...
Reads allowed API keys from MEDCOMPANION_API_KEYS environment variable (comma-separated).
Validates the X-MedCompanion-Key header on requests.
...
logger = logging.getLogger("medcompanion.integrations")

# Header name for API key
API_KEY_HEADER = "X-MedCompanion-Key"
...
def get_allowed_api_keys() -> Set[str]:
    ...
    """
    keys_raw = os.getenv("MEDCOMPANION_API_KEYS", "")
    if not keys_raw:
        logger.warning("MEDCOMPANION_API_KEYS not set - no integrations will be authorized")
        return set()
...
async def require_api_key(request: Request) -> str:
    """
    FastAPI dependency that validates the X-MedCompanion-Key header.
    ...
    if not api_key:
        ...
        raise HTTPException(
            ...
            detail="Missing X-MedCompanion-Key header",
        )
